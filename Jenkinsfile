pipeline {
    ...
    stages {
        stage('Git Checkout') {
            steps {
               git branch: 'main', credentialsId: 'git-cred', url: 'https://github.com/The-Cloud-Guy/netflixanalysis.git'
            }
        }
        ...
        stage('SonarQube Analysis') {
            steps {
                withSonarQubeEnv('sonar') {
                    sh ''' $SCANNER_HOME/bin/sonar-scanner -Dsonar.projectName=StockAnalysis \
                           -Dsonar.projectKey=StockAnalysis \
            ...
            steps {
               script {
                   withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker build -t thecloudguyn/netflixanalysis:latest ."
                    }
               }
            }
        }
        ...
        stage('Docker Image Scan') {
            steps {
                sh "trivy image --format table -o trivy-image-report.html thecloudguyn/netflixanalysis:latest"
            }
        }
        ...
        stage('Push Docker Image') {
            steps {
               script {
                   withDockerRegistry(credentialsId: 'docker-cred', toolName: 'docker') {
                        sh "docker push thecloudguyn/netflixanalysis:latest"
                    }
               }
            }
        }
            ...
            steps {
                script {
                    withKubeConfig(credentialsId: 'k8s') {
                        sh '''
                            helm upgrade --install netflixanalysis ./helm-Chart \
                            --set image.repository=thecloudguyn/netflixanalysis \
